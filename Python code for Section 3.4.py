#This file provides the python codes for Section 3.4
...
data_path = "E:/Data/With_Yang_Zhang/processed_data/"
...
######################
#This block builds and trains 9layer FNN bw_model9: C->vs
######################
...
fw_model.fit((train_x-param_m)/param_sd, train_y, epochs=800, 
          ...
          batch_size=15000, verbose=0, callbacks=[early_stop])
...
#The validation data is added the noise of Unif(-0.5%,0.5%)
#The training data is added the noise of Unif(-0.2%,0.2%), Unif(-0.5%,0.5%), Unif(-0.8%,0.8%), Unif(-1%,1%)
#We choose the noise added to the training data so that the model performs best on the noised val data
...
for delta in [0.002,0.005,0.008,0.01]:
    l = -delta
    h = delta
    np.random.seed(48)
    ...
    MDN_model = build_mdn(param_m = param_m,
                          param_sd = param_sd,
                          input_dim = 50,
                          hid_layers = (400,300,300,300,300),
                          num_mixes = 2,
                          out_dims = 9,    
                          w_regu = 0.00001,
    ...
    history = MDN_model.fit(x=(noised_train_x-den_m)/den_sd, y=train_dp, shuffle=True, 
                            batch_size=10000, epochs=1500,
                            validation_data=((noised_val_x-den_m)/den_sd,val_dp), verbose=0)
    print('delta=',delta, 'min val_loss=', min(history.history['val_loss']))
######################
##Constructs inputs and outputs to the FW-MDN (named as MDN_model) (9layer data)
...
noised_test_x = test_x*(1+noise[train_val_size:])
...
#########################
#Table 4
#Build and train the FW-MDN (named as MDN_model) : noised C -> several vs
            ...
            for w_reg in w_regu_list:
                for actv in activ_list:
                    for nm in n_mix_list:
                        ...
                        early_stop1 = EarlyStoppingByLossVal(monitor='val_loss', value=56000)
                        ...
                        noise4 = np.random.uniform(low=-0.008, high=0.008, size=test_x.shape)
                        ...
                        for k in range(5):
                            ...
                            x_R2 = r2_score(test_x[0:sample_size], x_pred)
                            print('R2 between fw_model.predict(ypred) and x_true:', x_R2)

###########
#For noised data - Unif(-0.9%, 0.9%)
...
noised_x = test_x*(1+noise)
#noised_x = test_x+noise

sample_size = test_x.shape[0]
mdn_output = MDN_model.predict((noised_x-den_m)/den_sd)
#mdn_output = MDN_model.predict((noised_test_x-den_m)/den_sd)
...
########################
#Fig.7
#Plot four randomly chosen x_pred, test_x, and noised_x (Also provides data for Fig.6)
########################
...
#ax[0,0].plot(np.arange(1,len(x1)+1),y1,color='r',label='dispersion curve true')
ax[0,0].scatter(np.arange(1,len(x1)+1),z1,color='r',s=10,label=r"$\bf{y}$"+"_noised")
ax[0,0].tick_params(labelsize=14)
ax[0,0].legend(loc='upper left',fontsize=15)


ax[0,1].plot(np.arange(1,len(x1)+1),x2,color='b',linestyle='-',label=r"$\bf{y}$"+"_pred")
#ax[0,1].plot(np.arange(1,len(x1)+1),y2,color='r',label='dispersion curve true')
ax[0,1].scatter(np.arange(1,len(x1)+1),z2,color='r',s=10,label=r"$\bf{y}$"+"_noised")
ax[0,1].tick_params(labelsize=14)
ax[0,1].legend(loc='upper left',fontsize=15)


ax[1,0].plot(np.arange(1,len(x1)+1),x3,color='b',linestyle='-',label=r"$\bf{y}$"+"_pred")
#ax[1,0].plot(np.arange(1,len(x1)+1),y3,color='r',label='dispersion curve true')
ax[1,0].scatter(np.arange(1,len(x1)+1),z3,color='r',s=10,label=r"$\bf{y}$"+"_noised")
...
#ax[1,1].plot(np.arange(1,len(x1)+1),y4,color='r',label='dispersion curve true')
ax[1,1].scatter(np.arange(1,len(x1)+1),z4,color='r',s=10,label=r"$\bf{y}$"+"_noised")
...
pd.DataFrame(np.append(test_y[idx_list],FWMDN_ypred[idx_list],axis=1)).to_csv("resforfig7.csv",index=False)